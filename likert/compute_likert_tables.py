...
LIKERT_CSV = os.path.join(REPORTS, "likert_correlations.csv")
CONT_CSV   = os.path.join(REPORTS, "standard_range_correlations.csv")
...
def load_likert():
    if not os.path.exists(LIKERT_CSV):
        raise SystemExit(f"{LIKERT_CSV} not found -- run likert/compute_likert_correlations.py first.")
    d = {}
...
def main():
    ...
    with open(os.path.join(REPORTS, "likert_table_values.csv"), "w", newline="") as f:
        w = csv.writer(f)
    ...
    with open(os.path.join(REPORTS, "likert_peak_values.csv"), "w", newline="") as f:
        w = csv.writer(f)
