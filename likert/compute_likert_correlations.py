...
def compute_corr(judge, dataset, metric, scale):
    ...
    if len(hum) < 2 or len(set(hum)) < 2 or len(set(pred)) < 2:
        return None
    return {
        "spearman": float(spearmanr(hum, pred)[0]),
        "kendall":  float(kendalltau(hum, pred)[0]),
        "n":        len(hum),
    }
...
def main():
    ...
    out = os.path.join(REPORTS, "likert_correlations.csv")
